...
export function ComparisonDisplay({ originalImage, protectedImage }) {
  const { t } = useLanguage();
  ...
  return (
    <div className="mb-12 animate-slide-up">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
        ...
        <Card className="overflow-hidden border-2 hover:border-primary/50 transition-all duration-300">
          <CardHeader className="bg-secondary py-3">
            <h3 className="text-center font-medium text-lg">{t('comparison.protected')}</h3>
          </CardHeader>
          <CardContent className="p-0">
            <div 
              className="relative h-80 cursor-zoom-in transition-transform duration-300 hover:scale-[1.02]" 
              onClick={() => setActiveImage('protected')}
            >
              <Image 
                src={protectedImage} 
                alt={t('comparison.protected')} 
                fill 
                style={{ objectFit: 'contain' }} 
              />
            </div>
      ...
      {activeImage && (
        ...
        >
          ...
          >
            <Image 
              src={activeImage === 'original' ? originalImage : protectedImage} 
              alt={activeImage === 'original' ? t('comparison.original') : t('comparison.protected')} 
              fill 
              style={{ objectFit: 'contain' }} 
